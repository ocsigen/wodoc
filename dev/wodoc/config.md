...
The left-column navigation is described declaratively here, and **this is the only way to define navigation**; the old wikicréole `menu.wiki` is gone. `(nav …)` holds an ordered list of sections:
...
- `(hosted (<pkg> <dir> <layout> <Wrapper>) …)`: cross-*project* references to another hosted Ocsigen project. odoc points these at `ocaml.org`; wodoc rewrites them to relative links into the sibling project's wodoc docs. `<layout>` is how that project is deployed: `multilib` (one `<pkg>.<lib>/` subtree per library, e.g. eliom), `root` (a single package at the version root) or `subdir` (a `<pkg>/` subtree per package, e.g. js\_of\_ocaml, tyxml); `true`/`false` are accepted as legacy aliases for `multilib`/`root`. `<Wrapper>` is its wrapper module, which that project deploys flat (`Eliom.Registration` *\-\>* `Eliom_registration`); pass `""` for a project that has none, and references then start with its own root module (`Js_of_ocaml.Js.t`). A reference to a `multilib` target from a page with no client/server side of its own links to the target's server library. Links go to the target's `latest`, except from a `dev` manual, which links to their `dev`.
