...
Only *repairable* references count. A reference into a dependency the site does not host (`Stdlib`, `Ppxlib`) is expected and nothing here can fix it, as is a project's reference to one of its own modules it does not publish; both are counted apart, on one line. What is reported is a cross-project page reference (a missing entry in `(hosted …)`, or a wrong page name), a reference to a page, value or type of this project that leads nowhere, and a module reference into a project the tables cover. A reference written with no label degrades to a bare `<code>`, indistinguishable from inline code, so only the labelled shape can be reported.
