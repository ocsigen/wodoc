  ...
  | _ :: "assemble" :: args -> (
      ...
      | Some tmpl, file :: _ ->
          ...
          let leftnav =
            ...
            | None -> ""
          in
          (* --mdlink <href>: advertise this page's Markdown twin with a
             <link rel="alternate" type="text/markdown"> (fills {{mdlink}}), so
             pages built via the low-level assemble path (e.g. the site vitrine)
             expose their .md version like the turn-key build does. *)
          let mdlink =
            match List.assoc_opt "mdlink" flags with
            | Some href ->
                Printf.sprintf
          ...
          let page =
            Wodoc.Assemble.page ~preamble ~flat
              ...
              ~menu_current ~leftnav ~mdlink ~template ~current (read_file file)
          in
            ...
            | Some cf -> (
              ...
              | Some b ->
                  ...
                  Wodoc.Blog.expand
                    ...
                    page)
          in
          (* --byline <text>: insert a styled byline (e.g. "date — author") right
             after the page title, matching the blog listing's meta line. *)
          let page =
            match List.assoc_opt "byline" flags with
            | None -> page
            | Some text ->
                Str.replace_first
                  (Str.regexp_string "</h1>")
                  (Printf.sprintf "</h1>\n<p class=\"wodoc-blog-meta\">%s</p>"
                     text)
                  page
          in
          print_string page
